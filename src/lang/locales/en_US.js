/* Gogo Language Texts

Table of Contents

01.General
02.User Login, Logout, Register
03.Menu
04.Error Page
*/

module.exports = {
  /* 01.General */
  'general.copyright': 'Gogo React © 2018 All Rights Reserved.',

  'unauthorized.title': 'Unauthorized Access Attempt',
  'unauthorized.detail':
    'You are not authorized to view the page you are trying to access.',

  /* 02.User Login, Logout, Register */
  'user.login-title': 'Login',
  'user.register': 'Register',
  'user.forgot-password': 'Forgot Password',
  'user.email': 'E-mail',
  'user.password': 'Senha',
  'user.forgot-password-question': 'Esqueceu sua senha?',
  'user.fullname': 'Full Name',
  'user.login-button': 'LOGIN',
  'user.register-button': 'REGISTER',
  'user.reset-password-button': 'RESET',
  'user.buy': 'BUY',
  'user.username': 'Username',
  'user.submit': 'Enviar',
  'user.current-password': 'Senha atual',
  'user.new-password': 'Nova senha',
  'user.new-password-again' : 'Digite novamente',

  /* 03.Menu */
  'menu.home': 'Home',
  'menu.app': 'Home',
  'menu.dashboards': 'Dashboards',
  'menu.gogo': 'Gogo',
  'menu.novo' : 'Novo',
  'menu.meu-perfil':'Meu Perfil',
  'menu.meus-agendamentos':'Meus agendamentos',
  'menu.usuario': 'Usuarios',
  'menu.start': 'Start',
  'menu.second-menu': 'Second Menu',
  'menu.second': 'Second',
  'menu.ui': 'UI',
  'menu.charts': 'Charts',
  'menu.chat': 'Chat',
  'menu.survey': 'Survey',
  'menu.todo': 'Todo',
  'menu.search': 'Search',
  'menu.docs': 'Docs',
  'menu.agendamentos': 'Agendamentos',
  'menu.list' : 'list',
  'menu.agendamento' : 'Agendamento',
  'menu.cliente' : 'Cliente',
  'pages.add-novo-agendamento': 'Novo Agendamento',
  'pages.add-novo-cliente': 'Novo Cliente',
  'pages.add-novo-colaborador': 'Novo Colaborador',
  'menu.colaboradores' : 'Colaboradores',
  'menu.clientes' : 'Clientes',
  'menu.novo-agendamento': 'Novo Agendamento',
  'menu.novo-cliente': 'Novo Cliente',
  'menu.novo-colaborador': 'Novo Colaborador',
  'menu.cancelar-agendamento': 'Cancelar',
  'menu.deletar-colaborador': 'Deletar',
  'menu.blank-page': 'Blank Page',
  'menu.form' : 'Form',
  /* 04.Error Page */
  'pages.error-title': 'Ooops... looks like an error occurred!',
  'pages.error-code': 'Error code',
  'pages.go-back-home': 'GO BACK HOME',
  'dashboards.calendar': 'Calendário de Agendamentos',
};
